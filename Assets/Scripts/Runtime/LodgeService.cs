...
namespace Height1079.Runtime
{
    ...
    public sealed class LodgeService : MonoBehaviour
    {
        ...
        {
            if (!ok) { CafeService.Purse.Refund(paid); note = text + " · деньги назад"; }
            else note = text;
        ...
        void Book()
        {
            ...
            if (paid > 0) return;
            if (!Lodging.Book(ref CafeService.Purse, near))
            {
                Say($"Не хватает денег: койка {near.Roubles} ₽, в кошельке {CafeService.Purse.Text}");
                Refresh();
                return;
            }
            paid = near.Roubles;
        ...
        void Refresh()
        {
            ...
            priceLine.text = near.Free
                ? $"{near.Ele:0} м · денег не берут · в кошельке {CafeService.Purse.Text}"
                : $"{near.Ele:0} м · {near.Roubles} ₽ за койку · в кошельке {CafeService.Purse.Text}";
            ...
            bool can = near.Free || CafeService.Purse.CanAfford(near.Roubles);
        ...
        string Line()
        {
            ...
            return near.Free
                ...
                : $"{near.Name} · E — лечь · {near.Roubles} ₽ · в кошельке {CafeService.Purse.Text}";
        }
