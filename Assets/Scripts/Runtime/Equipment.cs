...
namespace Height1079.Runtime
{
    ...
    public sealed class Equipment : MonoBehaviour
    {
        ...
        public const float BatterySeconds = TorchBeam.BatterySeconds;
        ...
        /// <summary>The light itself is shared with the sandbox's torch — see <see cref="TorchBeam"/>.</summary>
        TorchBeam beam;
        int beamKind = -1;
        ...
        public const float DynamoWind = TorchBeam.DynamoWind, DynamoRunDown = TorchBeam.DynamoRunDown;
        ...
        void LateUpdate()
        {
            ...
            if (beam != null && beam.Lit && beamOrigins[kind] != null)
        ...
        {
            if (beam == null) return;
            beam.Set(on, level, kind == 1);
            if (lensMats[kind] != null) lensMats[kind].SetColor("_EmissionColor", beam.LensEmission);
        }
    }
}
