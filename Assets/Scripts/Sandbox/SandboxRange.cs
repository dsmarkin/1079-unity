...
namespace Height1079.Sandbox
{
    ...
    /// metres. Everything is built from primitives at start-up; a stand is reached by its number key.</summary>
    public static class SandboxRange
    {
        ...
        static Material rock, ice, ledge, floor, mark;

        public static void Build()
        {
            Stands.Clear();
            Ramps.Clear();
            rock = Mat("rock", new Color(.44f, .42f, .40f));
            ice = Mat("ice", new Color(.68f, .80f, .86f));
            ledge = Mat("ledge", new Color(.52f, .48f, .38f));
            floor = Mat("floor", new Color(.74f, .76f, .78f));
            mark = Mat("mark", new Color(.85f, .55f, .2f));
        ...
        static void Ground()
        {
            var g = Box("Ground", new Vector3(0f, -1f, 0f), new Vector3(200f, 2f, 60f), floor);
        ...
        static void Steps(Vector3 at)
        {
            ...
            for (int i = 0; i < rise.Length; i++)
            {
                ...
                {
                    h += rise[i];
                    Box($"Step{i}_{s}", at + new Vector3(i * 3.2f, h * .5f, z + s * 1.2f), new Vector3(2.6f, h, 1.2f), ledge);
                }
                Sign($"{rise[i]:0.00} м", at + new Vector3(i * 3.2f, 2.2f, z - 1.4f));
            }
        ...
        static void Slopes(Vector3 at)
        {
            ...
            for (int i = 0; i < deg.Length; i++)
            {
                ...
                Sign($"{deg[i]:0}°", at + new Vector3(i * 2.6f, .6f, -1.6f));
            }
        ...
        static void Drops(Vector3 at)
        {
            ...
            for (int i = 0; i < h.Length; i++)
            {
                ...
                Sign($"{h[i]:0} м", at + new Vector3(i * 4f, h[i] + 1.2f, -1.8f));
            }
        ...
        static Material Mat(string name, Color c)
        {
            ...
            m.SetFloat("_Glossiness", name == "ice" ? .6f : .05f);
            return m;
        }
    }
}
