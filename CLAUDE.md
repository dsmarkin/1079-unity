...
## 6. Запуски Unity делает только ведущий агент

Один запуск Unity без окна (`check`, сборка, кадры) стоит 5–15 минут машинного времени, и параллельные агенты делят один компьютер. Поэтому:

- **Рабочие агенты (субагенты, участники параллельных задач) Unity не запускают вообще:** ни `check`, ни сборку, ни кадры. Они пишут код, данные и документацию, проверяют правила `dotnet run` в `Tools/CoreTests` (секунды) и отдают работу с отчётом: что сделано, что и где смотреть в кадрах, что не уверены.
- **Ведущий агент собирает один раз после слияния** работы всех: один `check` (компиляция, генерация, тесты) и одна сборка с кадрами. Ошибки компиляции и замечания по кадрам он раздаёт исполнителям одним пакетом; новый круг сборок — только после того, как весь пакет правок сделан.
- **Правки копить и делать пакетом,** а не «поправил мелочь — собрал — посмотрел». Что не вошло в пакет, пишется как известные недочёты.
- **Не плодить копии проекта.** Первая генерация в новой копии — 10 минут. Задачу продолжают в той же копии; новую заводят только под независимую задачу.
- **Настройки вида выносить в данные,** которые игра читает при запуске (JSON в `Resources`/`StreamingAssets`): цвета, масштабы, расстановку на площадке. Тогда их правят и пересматривают без пересборки.
- **Кадры смотрит человек один раз в конце,** по сборке ведущего агента.
