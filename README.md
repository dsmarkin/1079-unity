...
- Мир 4,1 × 4,1 км вокруг Холатчахля на рельефе ArcticDEM 2 м (Terrain 2049²), 36 тыс. деревьев на местах из модели высоты полога, курумы, реконструкции палатки, лабаза, кедра с костром и настила по документам, архивный слой (F2), осмотр мест событий камерой (F3). Снег: следы, тропа, метель. Снаряжение эпохи в руках — калька-карта, компас, фонарик ([docs/ITEMS.md](docs/ITEMS.md)). Данные, пайплайн и точность — в [docs/MAP.md](docs/MAP.md).
...
2. Первое открытие: пакеты подтянутся из манифеста, `ProjectSetup` создаст `Assets/Resources/{Hiker,NightSession}.prefab`, весь мир в `Assets/Resources/World` (1–2 минуты при первом открытии) и `Assets/Scenes/Main.unity`. Если что-то не появилось — меню **1079 → Generate prefabs and scene** или **1079 → Rebuild world**. Бинарные исходники (сетки рельефа, текстуры) лежат в git обычными файлами, 45 МБ суммарно.
3. Play. В меню: имя, «Создать ночь» (хост) или «Присоединиться» по адресу хоста. Второй экземпляр — через сборку или ParrelSync.

## Пролог

Сюжет, локации и участники миссий от Свердловска до палатки на склоне — в [docs/PROLOGUE.md](docs/PROLOGUE.md); посценный сценарий с локациями, референсами и репликами — в [docs/SCRIPT.md](docs/SCRIPT.md); бэклог механик в парадигме PEAK (тропёжка, палатка вдвоём, сушка, график, дежурство) — в [docs/BACKLOG.md](docs/BACKLOG.md). Данные ростера и миссий — `Assets/Scripts/Core/Campaign.cs` (с тестами хронологии); биты сцен, выборы, дневник и протокол пролога — `Assets/Scripts/Core/Prologue.cs` (`PrologueRun`: Advance/Choose → Protocol(), без движка). Реальные имена — только в прологе и архиве; ночь на склоне остаётся с вымышленной парой.

## Тесты

- В редакторе: Window → General → Test Runner → EditMode (тесты ядра, включая сверку карты с документированными расстояниями и высотами).
...
## Скрипты для Mac (`Tools/mac/`)

Запускаются двойным кликом из Finder (или из терминала), пишут логи в корень проекта; редактор берётся из `ProjectSettings/ProjectVersion.txt`.

- `check.command` — компиляция, генерация префабов/сцены, EditMode-тесты → `unity-check.log`, `unity-tests.xml`.
- `build.command` / `build-win.command` — сборка в `Builds/mac/1079.app` и `Builds/windows/1079.exe` → `build.log`, `build-win.log`.
- `run.command` — запуск Mac-сборки в окне 1280×800 с логом `player.log`.
